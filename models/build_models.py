...
def get_ortho_like(dim, alpha, beta, sign=1, dist='uniform'):
    ...
    if dist == 'uniform':
        A = alpha * np.random.uniform(size=(dim, dim), low=-3 ** 0.5 / (dim ** 0.5),
                                      high=3 ** 0.5 / (dim ** 0.5)) + sign * beta * np.eye(dim)
...
class KATVisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            weight_init: Literal[
                'skip', 'jax', 'jax_nlhb', 'moco', '', 'kan', 'kan_mimetic', 'mimetic', 'kan_mimetic2d'] = '',
            fix_init: bool = False,
        ...
        self.channels = [embed_dim] * depth # for segmentation tasks
...
def init_weights_attn_mimetic(module: nn.Module, name: str = '') -> None:
    ...
    if isinstance(module, Attention):
        ...
        for h in range(module.num_heads):
            ...
            module.qkv.weight.data[embed_dim + (h * head_dim):embed_dim + ((h + 1) * head_dim)] = torch.tensor(
                K.T).float()
...
def _create_kat_transformer(variant: str, pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                            **kwargs) -> KATVisionTransformer:
...
@register_model
def kat_tiny_patch16_224(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                         **kwargs) -> KATVisionTransformer:
...
@register_model
def kat_tiny_gelu_patch16_224(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                              **kwargs) -> KATVisionTransformer:
    """ KAT-Tiny with rational activations (ViT-S/16)
    """
    model_args = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3,
...
@register_model
def kat_tiny_swish_patch16_224(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                               **kwargs) -> KATVisionTransformer:
...
@register_model
def kat_small_patch16_224(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                          **kwargs) -> KATVisionTransformer:
...
@register_model
def kat_small_gelu_patch16_224(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                               **kwargs) -> KATVisionTransformer:
    """ KAT-Small with rational activations (ViT-S/16)"""
    model_args = dict(patch_size=16,
                      embed_dim=384,
...
@register_model
def kat_small_swish_patch16_224(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                                **kwargs) -> KATVisionTransformer:
    """ KAT-Small with rational activations (ViT-S/16)"""
    model_args = dict(patch_size=16,
                      embed_dim=384,
...
@register_model
def kat_base_patch16_224(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                         **kwargs) -> KATVisionTransformer:
...
@register_model
def kat_base_gelu_patch16_224(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                              **kwargs) -> KATVisionTransformer:
    """ KAT-Base with rational activations (ViT-B/16)"""
...
@register_model
def kat_base_swish_patch16_224(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                               **kwargs) -> KATVisionTransformer:
    """ KAT-Base with rational activations (ViT-B/16)"""
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12,
                      act_layer=KAT_Group,
...
#     net = kat_tiny_swish_patch16_224(num_classes=5)
